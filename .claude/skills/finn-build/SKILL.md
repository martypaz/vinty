...
Using the Linear connector, list issues on team `MAR` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `MAR-NNN-short-slug`, using the issue's real identifier.
...
- `Closes MAR-NNN`, using the real Linear issue identifier
